...
class NotificationsManager {
    ...
    //TODO: Test Local Notifications on physical device (search how to test)
    func testNotification(with name: String) {
        
        let calendar = Calendar.current
        guard let notificationDate = calendar.date(byAdding: .second, value: 10, to: .now) else { return }
        ...
        let dateComponents = calendar.dateComponents([.day, .month, .year, .minute, .second], from: notificationDate)
